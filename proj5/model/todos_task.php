...
class TasksFunc{
	...
	function get_owneremail($owneremail) {
		global $db;
		$query = 'SELECT * FROM todos WHERE owneremail = :owneremail ORDER BY duedate';
		$statement = $db->prepare($query);
	...
	public function get_isdone($email) {
		global $db;
		$query = 'SELECT `createddate`,`message`,`duedate`,`Title`, `id` FROM `todos` WHERE `isdone` = 0 AND `owneremail` = :email';
		$statement = $db->prepare($query);
	...
	public function get_iscompleted($email) {
		global $db;
		$query = 'SELECT `createddate`,`message`,`duedate`,`Title`, `id` FROM `todos` WHERE `isdone` = 1 AND `owneremail` = :email';
		$statement = $db->prepare($query);
        $statement->bindValue(':email', $_SESSION['uemail'] );
		$statement->execute();
